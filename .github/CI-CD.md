...
The public repository is a client distribution channel, not a contribution
surface: pull requests do not run this workflow. A qualifying push to protected
`main` validates Composer and runs PHPUnit, creates the next patch SemVer tag,
and sends a `repository_dispatch` event to PortalKit. A manual run may select a
`minor` or `major` increment; select `none` for a test-only run.
